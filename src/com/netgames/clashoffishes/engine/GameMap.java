...
public class GameMap {
    ...
    /**
     * Meant for debugging purpose, comment out to get rid of the black dots in
     * the actual game
     */
    ...
    /**
     * Generates the background for the canvas node. It does this by first
     * creating a gradient, then creating a rectangle that fills the given size
     * of the game map and then fills the rectangle with the created gradient.
     *
    ...
    /**
     * This method holds the algorithm that creates the background layers. It
     * does this by first defining a gradient for the layer. It then goes on to
     * set a number of variables to determine the size of the layer and its
     * boundaries. Based on these variables, a loop will iterate a number of
     * given times that adds lines to the GraphicsContext object, which closes
     * the path once the loop is done. This will create a layer shape that will
     * then be filled.
     *
     * @param gc A GraphicsContext object that issues draw calls to the canvas.
     * @param markers A list that contains all the line points of a given layer.
     * @param beginPoint The beginning point of the layer.
     * @param bias A differentiation item that allows the layers to lean more
     * towards going up or going down.
     * @param beginColor The begin color of the gradient, in hex.
     * @param endColor The end color of the gradient, in hex.
     */
    ...
    /**
     * Places a boat on a randomly chosen marker, based on a number of
     * conditions. These conditions being that the marker does not allow the
     * boat to be placed outside of the game field itself. Once a position is
     * decided upon, all the markers that fall within the boats reach, will be
     * marked is populated, so that no objects can be placed on top of the boat.
     *
     * @param gc A GraphicsContext object that issues draw calls to the canvas.
     * @param markers A list that contains all the line points of a given layer.
     */
     ...
     * corresponding markers to indicate where the markers are located. These
     * markers are used to place objects on top of them if they're free, else an
     * object cannot be placed on top of them. Note: This method is for
     * debugging purposes.
     *
     * @param gc A GraphicsContext object that issues draw calls to the canvas.
     * @param markers A list that contains all the line points of a given layer.
